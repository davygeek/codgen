...
func IsBeegoProject(thePath string) bool {
	...
	if err := <-c; err != nil {
		log.Fatalf("Unable to walk '%s' tree: %s", thePath, err)
	}
...
func FormatSourceCode(filename string) {
	...
	if err := cmd.Run(); err != nil {
		log.Warnf("Error while running gofmt: %s", err)
	}
}
...
func CheckEnv(appname string) (apppath, packpath string, err error) {
	...
	log.Warning("You current workdir is not inside $GOPATH/src.")
	log.Debugf("GOPATH: %s", FILE(), LINE(), gopath)
	...
	if _, e := os.Stat(apppath); !os.IsNotExist(e) {
		...
		log.Errorf("Path '%s' already exists", apppath)
		return
	}
